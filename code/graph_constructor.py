...
def graphs_from_mol_mul(dir, key, label, graph_dic_path, dis_threshold=8.0, path_marker='\\'):
    ...
    try:
        with open(dir, 'rb') as f:
            mol1, mol2, mol3, mol4 = pickle.load(f)
        # the distance threshold to determine the interaction between ligand atoms and protein atoms
        ...
        g2 = dgl.DGLGraph()
        # add nodes
        num_atoms_m1 = mol3.GetNumAtoms()  # number of ligand atoms
        num_atoms_m2 = mol4.GetNumAtoms()  # number of pocket atoms
        num_atoms = num_atoms_m1 + num_atoms_m2
        g2.add_nodes(num_atoms)

        if add_self_loop:
            nodes = g.nodes()
            g2.add_edges(nodes, nodes)

        # add edges, ligand molecule
        num_bonds1 = mol3.GetNumBonds()
        src1 = []
        dst1 = []
        for i in range(num_bonds1):
            bond1 = mol3.GetBondWithIdx(i)
            u = bond1.GetBeginAtomIdx()
            v = bond1.GetEndAtomIdx()
            src1.append(u)
            dst1.append(v)
        src_ls1 = np.concatenate([src1, dst1])
        dst_ls1 = np.concatenate([dst1, src1])
        g2.add_edges(src_ls1, dst_ls1)

        # add edges, pocket
        num_bonds2 = mol4.GetNumBonds()
        src2 = []
        dst2 = []
        for i in range(num_bonds2):
            bond2 = mol4.GetBondWithIdx(i)
            u = bond2.GetBeginAtomIdx()
            v = bond2.GetEndAtomIdx()
            src2.append(u + num_atoms_m1)
            dst2.append(v + num_atoms_m1)
        ...
        # add interaction edges, only consider the euclidean distance within dis_threshold
        g4 = dgl.DGLGraph()
        g4.add_nodes(num_atoms)
        dis_matrix = distance_matrix(mol3.GetConformers()[0].GetPositions(), mol4.GetConformers()[0].GetPositions())
        node_idx = np.where(dis_matrix < dis_threshold)
        src_ls3 = np.concatenate([node_idx[0], node_idx[1] + num_atoms_m1])
        dst_ls3 = np.concatenate([node_idx[1] + num_atoms_m1, node_idx[0]])
        g4.add_edges(src_ls3, dst_ls3)

        # assign atom features
        # 'h', features of atoms
        g2.ndata['h'] = torch.zeros(num_atoms, AtomFeaturizer.feat_size('h'))  # init 'h'
        g2.ndata['h'][:num_atoms_m1] = AtomFeaturizer(mol3)['h']
        g2.ndata['h'][-num_atoms_m2:] = AtomFeaturizer(mol4)['h']

        # assign edge features
        # 'd', distance between ligand atoms
        dis_matrix_L = distance_matrix(mol3.GetConformers()[0].GetPositions(), mol4.GetConformers()[0].GetPositions())
        m1_d = torch.tensor(dis_matrix_L[src_ls1, dst_ls1], dtype=torch.float).view(-1, 1)

        # 'd', distance between pocket atoms
        dis_matrix_P = distance_matrix(mol4.GetConformers()[0].GetPositions(), mol4.GetConformers()[0].GetPositions())
        m2_d = torch.tensor(dis_matrix_P[src_ls2 - num_atoms, dst_ls2 - num_atoms_m1], dtype=torch.float).view(-1, 1)
        ...
        # efeats1
        g2.edata['e'] = torch.zeros(g2.number_of_edges(), BondFeaturizer.feat_size('e'))  # init 'h'
        efeats1 = BondFeaturizer(mol3)['e']  
        g2.edata['e'][g2.edge_ids(src_ls1, dst_ls1)] = torch.cat([efeats1[::2], efeats1[::2]])

        # efeats2
        efeats2 = BondFeaturizer(mol4)['e']
        g2.edata['e'][g2.edge_ids(src_ls2, dst_ls2)] = torch.cat([efeats2[::2], efeats2[::2]])

        # 'e'
        g1_d = torch.cat([m1_d, m2_d])
        g2.edata['e'] = torch.cat([g2.edata['e'], g1_d * 0.1], dim=-1)
        g4.edata['e'] = g3_d * 0.1

        # if add_3D:
        # init 'pos'
        g2.ndata['pos'] = torch.zeros([g2.number_of_nodes(), 3])
        g2.ndata['pos'][:num_atoms_m1] = torch.tensor(mol3.GetConformers()[0].GetPositions(), dtype=torch.float)
        g2.ndata['pos'][-num_atoms_m2:] = torch.tensor(mol4.GetConformers()[0].GetPositions(), dtype=torch.float)
        # calculate the 3D info for g
        src_nodes, dst_nodes = g2.find_edges(range(g2.number_of_edges()))
        src_nodes, dst_nodes = src_nodes.tolist(), dst_nodes.tolist()
        neighbors_ls = []
    ...
    if status:
        with open(graph_dic_path + path_marker + key, 'wb') as f:
            pickle.dump({'g': g, 'g3': g3, 'g2':g2, 'g4':g4, 'key': key, 'label': label}, f)
...
def collate_fn_v2_MulPro(data_batch):
    ...
    :return:
    """
    graphs, graphs3, graphs2, graphs4, Ys, keys = map(list, zip(*data_batch))
    bg = dgl.batch(graphs)
    bg3 = dgl.batch(graphs3)
    bg2 = dgl.batch(graphs2)
    bg4 = dgl.batch(graphs4)
    Ys = torch.stack(Ys, dim=0)
    return bg, bg3, bg2, bg4, Ys, keys
...
class GraphDatasetMul_4(object):
    def __init__(self, labels, data_dirs, cache_file_path, add_3D):
...
class GraphDatasetV2MulPro(object):
    ...
    def __init__(self, keys, labels, data_dirs, graph_ls_path, graph_dic_path, num_process=6, dis_threshold=10.0,
    ...
    def _pre_process(self):
        if os.path.exists(self.graph_ls_path+self.path_marker+'g.bin'):
            ...
            with open(self.graph_ls_path + self.path_marker + 'g4.bin', 'rb') as f:
                self.graphs4 = pickle.load(f)    
            ...
            self.keys = sorted(
                key
                for key in os.listdir(self.graph_dic_path)
            ...
            for key in self.keys:
                with open(self.graph_dic_path + self.path_marker + key, 'rb') as f:
                    ...
                    self.graphs4.append(graph_dic['g4'])
            ...
            with open(self.graph_ls_path + self.path_marker + 'g4.bin', 'wb') as f:
                pickle.dump(self.graphs4, f)
        ...
        cmdline = 'rm -rf %s' % (self.graph_dic_path + self.path_marker + '*') 
        os.system(cmdline)

    def __getitem__(self, indx):
        return self.graphs[indx], self.graphs3[indx],  self.graphs2[indx],  self.graphs4[indx], torch.tensor(self.labels[indx], dtype=torch.long), self.keys[indx]

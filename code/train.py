#!/usr/bin/env python3
"""Train GPCRIGN on a simple train/validation/test split."""
...
DEFAULT_DATA_ROOT = Path("data") / "toy_example"
DEFAULT_LABEL_CSV = DEFAULT_DATA_ROOT / "label.csv"
DEFAULT_MODEL_DIR = Path("model_save")
DEFAULT_RESULT_DIR = Path("train_results")

DATASET_FOLDERS = {"train": "train", "validation": "validation", "test": "test"}
...
def main() -> None:
    ...
    train_loader, evaluation_loaders = create_data_loaders(args, label_dict)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        validation_mcc = epoch_metrics["validation"]["MCC"]
        if validation_mcc is None:
            raise ValueError("Validation set has no labels; early stopping cannot be evaluated.")
        if early_stopper.step(float(validation_mcc), model):
